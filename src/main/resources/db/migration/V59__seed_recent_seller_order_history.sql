-- V59: lịch sử đơn 06/08/2026 → CURRENT_DATE cho mọi seller đang bán.
--
-- Ngày + sự kiện (khớp DssHolidayCalendar 2026):
--   06/08 Thu     — ngày thường, giá niêm yết, biên lãi ổn
--   07–09/08      — ngày đôi 8/8 (mini sale 1.12): giảm giá, vài SKU bán lỗ nhẹ
--   10–19/08      — mùa tựu trường (1.14): laptop / phụ kiện / balo tăng
-- Cuối tuần tăng đơn; 1–2 ngày gần nhất còn PAID/PROCESSING/SHIPPING.
--
-- Idempotent: marker [RECENT-HIST]. Bỏ qua SP đã có dòng bán cùng ngày
-- (V46/V57) và SP dss-forecast-* (giữ profile xu hướng).
...
CREATE TEMP TABLE recent_buyers ON COMMIT DROP AS
SELECT u.id,
...
CREATE TEMP TABLE recent_catalog ON COMMIT DROP AS
SELECT
    p.id,
    p.seller_id,
    p.name,
    ...
    CASE
        WHEN COALESCE(c.slug, '') IN (
          ...
          OR p.name ILIKE '%tai nghe%'
          OR p.name ILIKE '%chuột%'
          OR p.name ILIKE '%bàn phím%'
            THEN 'ELECTRONICS'
        WHEN COALESCE(c.slug, '') IN ('kitchen', 'furniture', 'decor')
          OR p.name ILIKE '%nồi%'
          OR p.name ILIKE '%air fryer%'
            THEN 'HOME'
        WHEN COALESCE(c.slug, '') IN (
...
CREATE TEMP TABLE recent_days ON COMMIT DROP AS
...
CREATE TEMP TABLE recent_baskets ON COMMIT DROP AS
SELECT
    catalog.seller_id,
    day.sale_date,
    basket.basket_no,
    ...
    CASE
        ...
        ELSE 1.00::NUMERIC
    END AS event_mult
FROM (SELECT DISTINCT seller_id FROM recent_catalog) catalog
CROSS JOIN recent_days day
CROSS JOIN GENERATE_SERIES(1, 3) AS basket(basket_no)
WHERE EXISTS (SELECT 1 FROM recent_buyers)
  AND (
      basket.basket_no = 1
      OR (
          basket.basket_no = 2
          AND (
              EXTRACT(ISODOW FROM day.sale_date) IN (6, 7)
              OR day.sale_date BETWEEN DATE '2026-08-07' AND DATE '2026-08-09'
              OR (
                  day.sale_date BETWEEN DATE '2026-08-10' AND DATE '2026-08-25'
                  AND MOD(catalog.seller_id + EXTRACT(DOY FROM day.sale_date)::INTEGER, 2) = 0
              )
          )
      )
      OR (
          basket.basket_no = 3
          AND day.sale_date = DATE '2026-08-08'
      )
  );

CREATE TEMP TABLE recent_lines ON COMMIT DROP AS
SELECT
    basket.seller_id,
    basket.sale_date,
    basket.basket_no,
    ...
    GREATEST(
        ...
        ROUND(
            (1 + MOD(product.id, 3))
            * basket.event_mult
            * CASE WHEN basket.isodow IN (6, 7) THEN 1.35 ELSE 1.00 END
            * CASE
    ...
    CASE
        -- Flash 8/8: một số SKU bán dưới giá vốn ~5% (lỗ nhẹ, kéo đơn).
        WHEN basket.event_code = 'DOUBLE_88'
        ...
        ELSE product.price
    END AS unit_price
FROM recent_baskets basket
JOIN recent_catalog product ON product.seller_id = basket.seller_id
WHERE MOD(
          product.id
          + EXTRACT(DOY FROM basket.sale_date)::INTEGER * 3
          + basket.basket_no * 11,
          CASE WHEN basket.event_code = 'NONE' THEN 5 ELSE 3 END
      ) < 2
  AND NOT EXISTS (
      SELECT 1
      FROM order_items existing_item
      JOIN orders existing_order ON existing_order.id = existing_item.order_id
      WHERE existing_item.product_id = product.id
        AND existing_order.created_at::DATE = basket.sale_date
        ...
        AND existing_order.shipping_address NOT LIKE '[RECENT-HIST] %'
  );

CREATE TEMP TABLE recent_lines_capped ON COMMIT DROP AS
SELECT *
FROM (
    SELECT
        recent_lines.*,
...
SELECT
    ...
    CASE
        WHEN basket.sale_date = CURRENT_DATE THEN
            CASE MOD(basket.seller_id + basket.basket_no, 3)
                WHEN 0 THEN 'PAID'::order_status
                WHEN 1 THEN 'PROCESSING'::order_status
                ELSE 'PENDING'::order_status
            END
        WHEN basket.sale_date = CURRENT_DATE - 1 THEN 'SHIPPING'::order_status
        WHEN basket.sale_date = CURRENT_DATE - 2
             AND MOD(basket.seller_id + basket.basket_no, 4) = 0
            THEN 'SHIPPING'::order_status
        ...
        WHEN basket.sale_date BETWEEN DATE '2026-08-09' AND DATE '2026-08-11'
             AND MOD(basket.seller_id + basket.basket_no, 23) = 0
    ...
    '[RECENT-HIST] s'
        ...
        || basket.basket_no,
    basket.sale_date
        + CASE basket.basket_no
            WHEN 1 THEN TIME '09:20:00'
            WHEN 2 THEN TIME '14:45:00'
            ELSE TIME '20:10:00'
          END,
    basket.sale_date
        + CASE basket.basket_no
            WHEN 1 THEN TIME '18:00:00'
            WHEN 2 THEN TIME '19:30:00'
            ELSE TIME '22:00:00'
          END
FROM (
...
JOIN recent_buyers buyer
  ON buyer.rn = (
      MOD(
          ...
          + basket.basket_no
          - 1,
          (SELECT COUNT(*)::INTEGER FROM recent_buyers)
      ) + 1
  )
WHERE NOT EXISTS (
    SELECT 1
...
JOIN recent_lines_capped line
  ON demo_order.shipping_address =
        ...
        || ' #'
        || line.basket_no
WHERE demo_order.shipping_address LIKE '[RECENT-HIST] %'
  AND NOT EXISTS (
      SELECT 1
      FROM order_items existing_item
...
WHERE orphan.shipping_address LIKE '[RECENT-HIST] %'
  AND NOT EXISTS (
      SELECT 1
...
SET subtotal_amount = totals.subtotal,
    ...
    updated_at = demo_order.created_at + INTERVAL '6 hours'
FROM (
...
WHERE totals.order_id = demo_order.id
  AND demo_order.shipping_address LIKE '[RECENT-HIST] %';
...
SELECT
    ...
    END,
    'TXN_RECENT_' || demo_order.id,
    'VND',
    CASE
        WHEN demo_order.status = 'PENDING'::order_status THEN NULL
        WHEN demo_order.status = 'CANCELLED'::order_status THEN NULL
        ELSE demo_order.created_at + INTERVAL '12 minutes'
    END,
